...
{
    textcolor(15);
    int R = 1, z = 17, c, tecla; //variaveis para a configuração do jogo;
    char dif[10];
    do{
        system("cls");
        ...
        do{
            ...
            do{
                ...
                textbackground(15);
                textcolor(0);
                switch(z){ //cofiguração igual a de todos os menus com escolha;
    ...
    }while(R == 1);//controle para voltar ao inicio da escolha caso o usuario não aceite jogar com os valores escolhidos;
}
...
int testeBombas() //testa quantas bombas existem no tabuleiro, retormando esse valor para um teste;
{
    ...
    return c; //retorna o valor de c para verificar se a quantidade de bombas é a correta;
}
void randomMatriz() //randomiza posições da matris e atribui bombas;
{
    int x, y, i = 0, c = 0;
    srand(time(NULL)); //randomisa com base no horário do computador, desse modo a randomização nunca será igual;
    do{
        do{
            x = rand() % Linha;
            y = rand() % Coluna;
            Matriz1[x][y] = 9; //randomiza 2 coordenada para atribuir o 9 (numero escolhido para ser considerado uma bomba)
            i++; // incrementa i para não colocar mais bombas que o suficiente;
        }while(i < Bombas);
    c = testeBombas(); //recebe retorno de quantas bombas existem no tabuleiro;
    }while(c < Bombas || c > Bombas); //continua randomizando até que o numero de bombas seja o correto;
}
...
int testeZero(x, y) //faz um teste retornando 0 ou 1 para controle e execução do procedimento zero();
{
...
void zero(L, C)
{
    ...
    do{
        ...
        if(c == 1)
        {
            ...
            zero(L + 1, C); //executa o mesmo código para todas as direções (completamente à prova de erros)
            zero(L, C + 1);
            zero(L - 1, C);
            zero(L, C - 1);
        }
...
void reiniciarCantos() //atribui um valor não utilisado no jogo para a ultima coluna da Matriz1 para remover um erro de verificação;
{
    int x = 0;
    do{
...
void SalvarUltimoRank()
{
    ...
    for(i = 0; i <= 2; i++){ //for para salvar somente 3 letras na variavel;
        ...
        rRank[10].nick[i] = letra; // atribui as letras corretas para a variavel usuário, uma por uma;
...
void GAMEOVER()
{
    ...
    do{
        ...
        if(i == 18){ //caso sim irá executar o procedimento que salva o ranking;
            system("color 0F");
            SalvarUltimoRank();
            Controle = 1;
        }else{
...
void Sair() //procedimento de saída do jogo
{
    system("cls");
    printf("\n\n\tSaindo...\n\n\n\n\n\n\n\n\n\n\n\n\t");
    Sleep(3000);
...
void jogo()
{
    ...
    do{
       ...
       if (i != 1) {
           if(Matriz2[L][C]=='x' && Matriz1[L][C]!=9)//controle para não atribuir Score caso a posição já tenha sido escolhida;
           {
               Score=Score+dificuldade;
           }
           switch(Matriz1[L][C])
